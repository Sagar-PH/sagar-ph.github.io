...
document.addEventListener('scroll', () => {
    const mask_elems = document.getElementsByClassName('fade-in')

    Array.from(mask_elems).forEach((mask_elem) => {
        const mask_rect = mask_elem.getBoundingClientRect()
        const revealPoint = 50;

        if (mask_rect.top < (window.innerHeight - revealPoint)) {
            mask_elem.classList.add('in-view');
        }
    })
...
function adjustNavbar() {
    ...
    if (window.innerWidth < 481) {
        ...
        navbar.style.top = rect.height + 'px';
    } else {
